...
plrs.series <- function(
  ...
  if ("cghSeg" %in% class(cghseg)) {
    cghseg <- segmented(cghseg)
    cghcall <- NULL
  }
  if ("cghCall" %in% class(cghseg)) {
    cghcall <- calls(cghseg)
  ...
  if (length(control.select) > 0) {
    if (!"crit" %in% names(control.select)) {
      control.select$crit <- ifelse(control.model$constr, "osaic", "aic")
    }
  }
  # control.test
  ...
  nam3 <- colnames(Ieff) <- colnames(Seff) <- c(
    "Loss",
    "Norm.",
    "Gain",
    "Ampl."
  ...
  if (control.test$testing) {
    res.test <- matrix(
      ...
      ngenes,
      3,
      dimnames = list(rownames(est), c("stat", "raw.pval", "BH.adj.pval"))
    )
  }
  ...
  while (sum(ind) != 0) {
    ...
    ind <- duplicated(lab)
    if (sum(ind) != 0) {
      lab[ind] <- substr(lab[ind], 1, nchar(lab[ind]) - 2)
    }
    cp <- cp + 1
  }
  ...
  for (x in 1:ngenes) {
    ...
    if (!inherits(error, "try-error")) {
      ...
      if (control.output$save.models) {
        save(
          model,
          file = paste("plrsSeriesObjects/gene_", lab[x], ".RData", sep = "")
        )
      }
      if (control.output$plot.lin || control.output$save.plots) {
        if (control.output$save.plots) {
          if (control.output$type == "pdf") {
            pdf(paste("plrsSeriesPlots/gene", lab[x], ".pdf", sep = ""))
          }
          if (control.output$type == "jpeg") {
            jpeg(paste("plrsSeriesPlots/gene", lab[x], ".jpeg", sep = ""))
          }
        }
