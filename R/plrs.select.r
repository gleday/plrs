...
plrs.select <- function(
  object,
  crit = ifelse(object@call.arg$constr, "osaic", "aic")
) {
  criter <- crit
  if (is.null(criter)) {
    stop("No criterion")
  ...
  if (length(criter) > 1) {
    stop("Multiple criteria are not allowed")
  }
  if (!is.character(criter)) {
    stop("Criterion has to be a character")
  }
  if (!criter %in% c("aic", "aicc", "bic", "osaic")) {
  ...
  for (k in 1:length(allsubmodels$allMats)) {
    allModels[[k]] <- .plrs.fit(
      X = allsubmodels$allMats[[k]],
      matconstr = allsubmodels$allConstr[[k]],
      mytype = allsubmodels$allTypes[k],
      modelFull = object,
      callArg = object@call.arg
    )
  }
  ...
  out <- new("plrs.select", "table" = crits, "model" = bmodel, "crit" = criter)
  return(out)
}
