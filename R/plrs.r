...
plrs <- function(
  ...
  ## check input data
  if (!is.vector(expr)) {
    stop("expr must be a vector")
  }
  if (!is.vector(cghseg)) {
    stop("cghseg must be a vector")
  }
  if (any(is.na(expr))) {
    stop("Missing values are not allowed in expr")
  }
  if (any(is.na(cghseg))) {
    stop("Missing values are not allowed in cghseg")
  }
  if (is.null(cghcall)) {
    cghcall <- rep(0, length(cghseg))
  } else {
    if (!is.vector(cghcall)) {
      stop("cghcall must be a vector")
    } else {
      if (any(is.na(cghcall))) {
        stop("Missing values are not allowed in cghcall")
      }
  ...
  if (length(expr) != length(cghseg)) {
    stop("expr and cghseg are incompatible")
  }
  if (length(expr) != length(cghcall)) {
    stop("expr and cghcall are incompatible")
  }
  if (length(cghseg) != length(cghcall)) {
    stop("cghseg and cghcall are incompatible")
  }
  if (!is.null(probloss) & !is.vector(probloss)) {
    stop("probloss must be a vector")
  }
  if (!is.null(probnorm) & !is.vector(probnorm)) {
    stop("probnorm must be a vector")
  }
  if (!is.null(probgain) & !is.vector(probgain)) {
    stop("probgain must be a vector")
  }
  if (!is.null(probamp) & !is.vector(probamp)) {
    stop("probamp must be a vector")
  }
  ...
  nval <- length(val)
  if (nval > 4) {
    stop("Configurations with more than 4 states are not allowed")
  }
  out <- NULL

  ## Check configuration
  # min.obs
  if (!is.numeric(min.obs)) {
    stop("'min.obs' must be a numeric")
  }
  min.obs <- round(min.obs)
  if (min.obs < 1) {
    stop("'min.obs' must be a positive integer")
  }
  # discard.obs
  if (!is.logical(discard.obs)) {
    stop("'discard.obs' must be a logical")
  }
  # Nb of obs
  ...
  if (any(is.na(mcghcall))) {
    ...
    if (!is.null(mprobloss)) {
      mprobloss <- probloss[!is.na(mcghcall)]
    }
    if (!is.null(mprobnorm)) {
      mprobnorm <- mprobnorm[!is.na(mcghcall)]
    }
    if (!is.null(mprobgain)) {
      mprobgain <- mprobgain[!is.na(mcghcall)]
    }
    if (!is.null(mprobamp)) {
      mprobamp <- mprobamp[!is.na(mcghcall)]
    }
  ...
  if (nval != 1) {
    ...
    if (is.null(knots)) {
      ...
      if (sum(order(chg.pt) == 1:length(chg.pt)) != length(chg.pt)) {
        warning(
          "Cannot fit a plrs model: estimated knots are non-ordered (this may be caused by strong outliers)."
        )
      }
      # Recalling
      cl <- rep(val[1], length(mcghcall))
      if (length(chg.pt) >= 1) {
        cl[mcghseg >= chg.pt[1]] <- val[2]
      }
      if (length(chg.pt) >= 2) {
        cl[mcghseg >= chg.pt[2]] <- val[3]
      }
      if (length(chg.pt) >= 3) {
        cl[mcghseg >= chg.pt[3]] <- val[4]
      }
      ...
      if (!is.numeric(knots)) {
        stop("'knots' is not correct")
      }
      if (length(knots) != (nval - 1)) {
        stop("wrong number of knots")
      }
      chg.pt <- knots
    }
    ...
    if (
      is.null(continuous) ||
        ...
        (is.vector(continuous) & length(continuous) > 1)
    ) {
      stop("'continuous' is not correct")
    }

    # constr
    if (!(is.logical(constr) & length(constr) == 1)) {
      stop("'constr' must be a logical")
    } else {
      # constr.slopes
  ...
  if (is.piecewise.level(sol)) {
    model.type <- "Piecewise level"
  }
  if (is.piecewise.linear(sol)) {
