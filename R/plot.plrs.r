...
plot.plrs <- function(
  ...
  nval <- length(val)
  pl <- FALSE
  if (x@type == "Linear" || x@type == "Intercept") {
    pl <- TRUE
  }
  if (!is.vector(col.pts)) {
    stop("col.pts is not correct")
  }
  ...
  if (length(col.pts) > 1) {
    for (i in val) {
      colr[called == i] <- col.pts[c(-1, 0, 1, 2) == i]
    }
  }
  if (length(pch) > 1) {
    if (length(pch) != 4) {
      stop("Either one or four values as input")
    }
    pchtp <- rep(pch[1], length(called))
    for (i in val) {
      pchtp[called == i] <- pch[c(-1, 0, 1, 2) == i]
    }
  } else {
  ...
  if (!pl) {
    xx <- NULL
    if (!is.null(knots(x))) {
      xx <- c(knots(x) + 1e-5, knots(x) - 1e-5)
    }
    xx <- c(
      xx,
      seq(
        from = xlim[1] - 0.5,
        to = xlim[2] + 0.5,
        length = 50 - length(knots(x))
      )
    )
    if (!is.null(knots(x))) {
      xx[xx %in% knots(x)] <- xx[xx %in% knots(x)] + 1e-4
    }
    xx <- sort(xx)
    ...
    while (bool) {
      ...
      if (ct <= nknots) {
        ...
        if (length(x@cb) != 0) {
          ...
          polygon(
            c(xCB[indCB], xCB[indCB][length(xCB[indCB]):1]),
            c(ylbCB[indCB], yubCB[indCB][length(yubCB[indCB]):1]),
            col = col.cb,
            border = NA
          )
          xCB[indCB] <- NA
    ...
    if (length(x@cb) != 0) {
      polygon(
        c(xCB, xCB[length(xCB):1]),
        c(ylbCB, yubCB[length(yubCB):1]),
        col = col.cb,
        border = NA
      )
    ...
    if (length(coef(x)) == 1) {
      abline(a = coef(x)[1], b = 0, lwd = lwd, col = col.line, lty = lty)
    }
  }
}
