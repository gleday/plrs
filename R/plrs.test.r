...
plrs.test <- function(object, alpha = 0.05) {
  ...
  if (length(coef(obj)) != 1) {
    if (obj@call.arg$constr) {
      # Test whether all inequalities are equalities
      ...
      stat <- as.vector(crossprod(
        coef(obj) - b.eq,
        solve(cov.mat, coef(obj) - b.eq)
      ))
      ...
      pval <- 1 -
        pbetabar(
          stat,
          df1 = df.bar / 2,
          df2 = unconstr@QP$df.error / 2,
          wt = wt.bar
        )
      qbetabar <- .quantileBetabar(
        df.bar = df.bar,
        df.error = unconstr@QP$df.error,
        wt.bar = wt.bar,
        alpha = alpha
      )
      test <- list(
