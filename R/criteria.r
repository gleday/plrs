...
criteria <- function(
  ...
  # Error handling
  if (!"plrs" %in% class(obj)) {
    stop("An object of class \"plrs\" is required")
  }
  if (length(crit) == 0) {
    stop("No criterion")
  }
  if (length(crit) == 1) {
    if (crit == "all") crit <- c("aic", "aicc", "bic", "osaic")
  }
  if (!is.character(crit)) {
    stop("'crit' has to be a character")
  }
  if (
    !"all" %in% crit &
      !"aic" %in% crit &
    ...
    stop("Wrong criterion")
  }
  bool <- TRUE
  if ("osaic" %in% crit) {
    if (!obj@call.arg$constr) {
      bool <- FALSE
    } else {
  ...
  if (rss != 0) {
    if (obj@call.arg$constr & length(coef(obj)) > 1) {
      p <- length(coef(obj)) - obj@QP$meq
    } else {
      p <- length(coef(obj))
    }

    if ("aic" %in% crit || "aicc" %in% crit || !bool) {
      aic <- n * log(rss / n) + 2 * p
    }
    if ("aicc" %in% crit) {
    ...
    if ("osaic" %in% crit) {
      # As defined in Hughes et al (2003)
      if (bool) {
        # Calculate weights
        obj.unconstr <- .plrs.fit(
          X = obj@X,
          matconstr = NULL,
          mytype = obj@type,
          modelFull = obj,
          callArg = obj@call.arg
        )
        ...
        if (obj@QP$meq == 0) {
          ...
          wt.bar <- ic.weights(solve(solve(crossprod(
            t(crossprod(t(uiw), cov.mat)),
            t(uiw)
          ))[-(1:obj@QP$meq), -(1:obj@QP$meq)]))
        }
  ...
  out <- NULL
  if ("aic" %in% crit) {
    out <- c(out, list("aic" = aic))
  }
  if ("aicc" %in% crit) {
    out <- c(out, list("aicc" = aicc))
  }
  if ("bic" %in% crit) {
    out <- c(out, list("bic" = bic))
  }
  if ("osaic" %in% crit) {
    out <- c(out, list("osaic" = osaic))
  }
  return(out)
}
